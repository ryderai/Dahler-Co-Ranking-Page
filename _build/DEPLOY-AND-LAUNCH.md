# Deploy and launch — The 30A Report
...
Ryder picked **the30areport.com** on 17 Sep 2026. RDAP (the .com registry's own lookup) returned
"not found" for it that night = unregistered at that moment. Confirm and buy at GoDaddy before step 2.
Fallbacks checked the same way, also unregistered: 30ateamreport.com, 30aluxuryindex.com.

## 2. Point the site at it

```
The build already points at https://the30areport.com (bare domain, no www). If you keep that, skip this.
Otherwise: bash _build/set-domain.sh https://www.the30areport.com
```

One command. It rewrites every canonical, og:url, sitemap entry, llms.txt link and markdown
mirror, rebuilds all 18 pages and runs verify.py. If verify does not print ALL PASS, stop.
...
```
python3 _build/verify.py https://the30areport.com
```
...
## 7. The one link that matters

Nothing links to a new ranking site. The first inbound link has to come from the client's own site:
one line on 30arealestatefl.com (About page or footer) — "Ranked #1 luxury team on 30A by The 30A
